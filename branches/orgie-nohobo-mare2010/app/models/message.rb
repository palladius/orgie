class Message < ActiveRecord::Base
  ...
  def father
    return [] unless son?
    Message.find(self.id_figliodi_msg)
  end
  
    # returns array of sons
  def sons
    return [] unless father?
    Message.find_all_by_id_figliodi_msg(self.id)
  end
  ...
  def creation_date
    (self.data_creazione || Time.now ) # rescue "_BOH_DATE_AGO_(#{$!})_" # _BOH_DATE_AGO_
  end
  ...
  def validate
    #    if id==0 then
    errors.add( :id, " ID cannot be zero, sorry! Or it would be a legal father for every 'father' message :) Start from 1 instead.." ) if id==0
    #    end
  end
